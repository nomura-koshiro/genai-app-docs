...
使用方法:
    >>> from app.schemas.analysis_session import AnalysisSessionCreate, ChatRequest
    >>>
    >>> # セッション作成
...
class AnalysisSessionDetailResponse(AnalysisSessionResponse):
    ...
    Attributes:
        ...
        snapshot_history (list[dict[str, Any]] | None): スナップショット履歴
    ...
    snapshot_history: list[dict[str, Any]] | None = Field(None, description="スナップショット履歴")
...
class AnalysisStepResponse(AnalysisStepBase):
    ...
    Attributes:
        ...
        result_formula (list[dict[str, Any]] | None): 結果数式
        is_active (bool): アクティブフラグ
        created_at (datetime): 作成日時
        updated_at (datetime): 更新日時
    """
    ...
    result_formula: list[dict[str, Any]] | None = Field(None, description="結果数式のリスト")
    is_active: bool = Field(..., description="アクティブフラグ")
    created_at: datetime = Field(..., description="作成日時（UTC）")
    updated_at: datetime = Field(..., description="更新日時（UTC）")
...
class AnalysisFileResponse(AnalysisFileBase):
    ...
    file_metadata: dict[str, Any] | None = Field(None, description="その他のメタデータ")
    is_active: bool = Field(..., description="アクティブフラグ")
    created_at: datetime = Field(..., description="作成日時（UTC）")
    updated_at: datetime = Field(..., description="更新日時（UTC）")
