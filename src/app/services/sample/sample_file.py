...
# 注意: MAX_FILE_SIZEはsettings.MAX_FILE_SIZE_BYTESから取得されます
...
class SampleFileService:
    ...
    async def upload_file(self, file: UploadFile, user_id: uuid.UUID | None = None) -> SampleFile:
        ...
        try:
            ...
            if file_size > settings.MAX_FILE_SIZE_BYTES:
                raise ValidationError(
                    f"ファイルサイズが大きすぎます（最大: {settings.MAX_FILE_SIZE_MB}MB）",
                    details={"size": file_size, "max_size": settings.MAX_FILE_SIZE_BYTES},
                )
        finally:
            await file.seek(0)  # ファイルポインタをリセット（必ず実行）
    ...
    async def list_files(self, user_id: uuid.UUID | None = None, skip: int = 0, limit: int = 100) -> list[SampleFile]:
        ...
        """
        logger.info("ファイル一覧取得", user_id=user_id, skip=skip, limit=limit)

        files = await self.repository.list_files(user_id=user_id, skip=skip, limit=limit)

        logger.info("ファイル一覧取得完了", count=len(files))
