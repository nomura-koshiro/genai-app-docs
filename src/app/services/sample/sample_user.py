...
class SampleUserService:
    ...
    async def get_user(self, user_id: uuid.UUID) -> SampleUser | None:
        ...
        """
        logger.info("ユーザーIDでユーザーを取得中", user_id=user_id, action="get_user")
        ...
        logger.info("ユーザーを正常に取得しました", user_id=user.id, email=user.email)
        return user
    ...
    async def list_users(self, skip: int = 0, limit: int = 100) -> list[SampleUser]:
        ...
        """
        logger.info("ユーザー一覧を取得中", skip=skip, limit=limit, action="list_users")
