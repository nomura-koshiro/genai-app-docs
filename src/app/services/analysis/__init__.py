...
class AnalysisService:
    ...
    async def create_session(
        self, session_data: AnalysisSessionCreate, creator_id: uuid.UUID
    ) -> AnalysisSession:
        """新しい分析セッションを作成します。"""
    ...
    async def upload_data_file(
        ...
        return await self._file_service.upload_data_file(
            session_id, file_request, user_id
        )

    async def list_session_files(
        self, session_id: uuid.UUID, is_active: bool | None = None
    ) -> list[AnalysisFile]:
        """セッションのファイル一覧を取得します。"""
        return await self._file_service.list_session_files(session_id, is_active)

    async def create_step(
        self, session_id: uuid.UUID, step_data: AnalysisStepCreate
    ) -> AnalysisStep:
        """新しい分析ステップを作成します。"""
    ...
    async def execute_chat(
        self, session_id: uuid.UUID, chat_request: ChatRequest
    ) -> ChatResponse:
    ...
    async def save_snapshot(
        ...
        return await self._snapshot_service.save_snapshot(
            session_id, user_id, current_snapshot
        )

    async def revert_snapshot(
        self, session_id: uuid.UUID, user_id: uuid.UUID, snapshot_id: int
    ) -> None:
        """指定したスナップショットに戻します。"""
        await self._snapshot_service.revert_snapshot(session_id, user_id, snapshot_id)

    async def restore_snapshot(
        self, session_id: uuid.UUID, snapshot_id: int
    ) -> AnalysisSession:
        """スナップショットから状態を復元します（廃止予定）。"""
        return await self._snapshot_service.restore_snapshot(session_id, snapshot_id)
