...
class ToolTrackingHandler(BaseCallbackHandler):
    ...
    Attributes:
        tool_usage (list[dict[str, Any]]): ツール使用履歴のリスト
            - tool: ツール名
            - input: ツール入力
            - output: ツール出力
    ...
    def on_tool_start(
        ...
        self.tool_usage.append({"tool": tool_name, "input": input_str, "output": None})
...
"""
    ...
    async def _save_chat_history(self) -> None:
        ...
        ConversationBufferMemoryの会話履歴を
        AnalysisSession.chat_historyに保存します。

        Note:
            - HumanMessage → user, AIMessage → assistant に変換
            - snapshot_idは現在のスナップショットIDを使用
        """
        try:
            if not self.memory:
                return
    ...
    async def chat(self, user_input: str, max_retry: int = 3) -> str:
        ...
        if not self.agent_executor:
            raise ValidationError(
                "エージェントが初期化されていません。initialize()を呼び出してください。"
            )
...
"""
                ...
                response = await self.agent_executor.ainvoke(
                    {"input": user_input}, callbacks=[handler]
                )
                ...
                if "400" in str(e) and attempt < max_retry - 1:
                    ...
                    error_chat_history.append(
                        {
                            "role": "assistant",
                            "message": f"⚠️ 実行中にエラーが発生しました: {str(e)}。再開します。(試行 {attempt + 1}/{max_retry})",
                            "snapshot_id": snapshot_id,
                        }
                    )
                    await self.session_repository.update(
                        session, chat_history=error_chat_history
                    )
