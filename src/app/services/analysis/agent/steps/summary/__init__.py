...
# 対応している数式タイプ
SUPPORTED_FORMULA_TYPES = ["sum", "mean", "count", "max", "min", "+", "-", "*", "/"]
...
class SummaryStep(BaseAnalysisStep):
    ...
    async def validate_config(
        ...
        if "formula" in config:
            ...
            if not isinstance(formulas, list):
                raise ValidationError(
                    "formulaはlistである必要があります",
                    details={"formula": formulas},
                )

            for i, formula in enumerate(formulas):
                if not isinstance(formula, dict):
                    raise ValidationError(
                        f"formula[{i}]はdictである必要があります",
                        details={"formula": formula},
                    )

                # 必須フィールド
                required_fields = ["target_subject", "type", "formula_text"]
                for field in required_fields:
                    if field not in formula:
                        raise ValidationError(
                            f"formula[{i}]に{field}が必要です",
                            details={"formula": formula},
                        )

                # typeの検証
                formula_type = formula["type"]
                if formula_type not in SUPPORTED_FORMULA_TYPES:
                    raise ValidationError(
                        f"未対応のformula typeです: {formula_type}",
                        details={"type": formula_type, "supported": SUPPORTED_FORMULA_TYPES},
                    )
        ...
        if "chart" in config:
            ...
            if not isinstance(chart, dict):
                raise ValidationError(
                    "chartはdictである必要があります",
                    details={"chart": chart},
                )

            # graph_typeの検証
            if "graph_type" in chart:
                graph_type = chart["graph_type"]
    ...
    async def execute(
        ...
        **kwargs: Any,
        ...
        # 数式計算
        if "formula" in config:
            result_formula = await self._apply_formula(source_data, config["formula"])
            logger.debug("数式計算完了", formulas_count=len(result_formula))

        # グラフ生成
        if "chart" in config:
            result_chart = await self._apply_chart(source_data, config["chart"])
            logger.debug("グラフ生成完了", graph_type=config["chart"].get("graph_type"))
    ...
    async def _apply_formula(
        ...
        Example:
            ...
            >>> print(result)
            [{"name": "売上合計", "result": 1000000, "unit": "円"}]
        """
        ...
        for formula in formulas:
            ...
            portion = formula.get("portion", 1.0)  # 重み係数
