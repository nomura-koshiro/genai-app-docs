...
class AggregationStep(BaseAnalysisStep):
    ...
    async def validate_config(
        ...
        if "axis" not in config:
            raise ValidationError("axisが必要です", details={"config": config})

        if "column" not in config:
            raise ValidationError("columnが必要です", details={"config": config})
    ...
    async def execute(
        ...
        **kwargs: Any,
        ...
        logger.info(
            ...
            axis=config["axis"],
            columns_count=len(config["column"]),
        )
        ...
        result_df = pd.DataFrame(index=grouped.groups.keys())
