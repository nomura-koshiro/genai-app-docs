...
class AnalysisDataManager:
    ...
    @measure_performance
    async def upload_data(self, upload_file: dict[str, Any] | UploadFileData) -> None:
        """アップロードされたデータを登録します。
        ...
        Args:
            upload_file (dict[str, Any] | UploadFileData): アップロードされたファイルの情報
        ...
        if isinstance(upload_file, dict):
            try:
                file_data = UploadFileData.model_validate(upload_file)
        ...
        try:
            # DataFrameの検証
            if not isinstance(file_data.data, pd.DataFrame):
                raise ValidationError(
                    "dataはpd.DataFrameである必要があります",
                    details={"type": type(file_data.data).__name__},
                )
            ...
            if not existing_file:
                ...
                await self.file_repository.create(
                    ...
                    table_axis=file_data.table_axis,
                    metadata={
                        "row_count": len(df),
