...
@project_files_router.get(
    "/project/{project_id}/file/{file_id}/version/{version_id}",
    response_class=FileResponse,
    summary="特定バージョンダウンロード",
    description="""
    ファイルの特定バージョンをダウンロードします。

    **認証が必要です。**

    パスパラメータ:
        - project_id: uuid - プロジェクトID（必須）
        - file_id: uuid - ファイルID（必須、バージョン管理のため）
        - version_id: uuid - ダウンロードするバージョンのファイルID（必須）

    レスポンス:
        - バイナリストリーム（ファイルデータ）
        - Content-Disposition: attachment; filename="filename_v{version}.ext"
...
async def download_file_version(
    ...
    # バージョンファイルのパスを取得
    filepath = await file_service.download_version(version_id, user_id)

    # ファイルメタデータを取得（ファイル名とバージョン番号用）
    file_metadata = await file_service.get_file(version_id, user_id)

    # ファイル名にバージョン番号を付与
    original_name = file_metadata.original_filename
    name_parts = original_name.rsplit(".", 1)
    if len(name_parts) == 2:
        versioned_filename = f"{name_parts[0]}_v{file_metadata.version}.{name_parts[1]}"
    else:
        versioned_filename = f"{original_name}_v{file_metadata.version}"

    logger.info(
        "ファイルバージョンをダウンロードしました",
        version_id=str(version_id),
...
@project_files_router.post(
    "/project/{project_id}/file/{file_id}/version/{version_id}/restore",
    response_model=FileVersionRestoreResponse,
    summary="バージョン復元",
    description="""
    ファイルの特定バージョンを最新として復元します（実際は新バージョンとして登録）。
    ...
    **認証が必要です。**
    ...
    パスパラメータ:
        - project_id: uuid - プロジェクトID（必須）
        - file_id: uuid - ファイルID（必須、バージョン管理のため）
        - version_id: uuid - 復元元のバージョンファイルID（必須）

    リクエストボディ:
        - FileVersionRestoreRequest:
            - comment: str | None - 復元コメント（省略時は自動生成）

    レスポンス:
        - FileVersionRestoreResponse: 復元レスポンス
            - file_id (uuid): 最新バージョンのファイルID
            - new_version_id (uuid): 新しく作成されたバージョンのID
...
async def restore_file_version(
    ...
    result = await file_service.restore_version(version_id, user_id, request.comment)
...
@project_files_router.get(
    "/project/{project_id}/file/{file_id}/version/compare",
    response_model=FileVersionCompareResponse,
    summary="バージョン比較",
    description="""
    ファイルの2つのバージョンを比較します（ファイルサイズ比較のみ）。
    ...
    **認証が必要です。**
    ...
    クエリパラメータ:
        - version1: int - 比較元バージョン番号（必須）
        - version2: int - 比較先バージョン番号（必須）
...
async def compare_file_versions(
    ...
    file_service: ProjectFileServiceDep,
    current_user: CurrentUserAccountDep,
    version1: int = Query(..., description="比較元バージョン番号"),
    version2: int = Query(..., description="比較先バージョン番号"),
) -> FileVersionCompareResponse:
    """ファイルの2つのバージョンを比較します。"""
    logger.info(
        "ファイルバージョン比較リクエスト",
        ...
        version2=version2,
        user_id=str(current_user.id),
        action="compare_file_versions",
    )

    user_id = current_user.id

    result = await file_service.compare_versions(file_id, version1, version2, user_id)
    ...
    logger.info(
        ...
        version1=version1,
        version2=version2,
        size_change=result.comparison.size_change,
        size_change_percent=result.comparison.size_change_percent,
    )
