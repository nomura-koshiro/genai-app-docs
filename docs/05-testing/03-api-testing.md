...
class TestFileList:
    ...
    def test_list_files_with_pagination(self, client, sample_files):
        """ページネーション付きファイル一覧"""
        # 最初の2件
        response = client.get("/api/files/list?skip=0&limit=2")
        ...
        # 次の2件
        response = client.get("/api/files/list?skip=2&limit=2")
    ...
    def test_list_files_pagination_variations(
        ...
        """様々なページネーションパターン"""
        response = client.get(f"/api/files/list?skip={skip}&limit={limit}")
...
def test_download_file_with_invalid_id(client):
    """無効なIDでファイルダウンロード"""
    response = client.get("/api/files/download/invalid-id")
...
class TestErrorResponses:
    ...
    def test_not_found_error_response(self, client):
        """NotFoundエラーのレスポンス"""
        response = client.get("/api/files/download/nonexistent-id")
