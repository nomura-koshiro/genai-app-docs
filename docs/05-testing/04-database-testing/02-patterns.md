...
このドキュメントでは、データベーステストの具体的なパターン、よくある問題と解決策、ベストプラクティスについて説明します。

[← 前へ: テストデータベースのセットアップ](./04-database-testing-setup.md) | [↑ データベーステスト](./04-database-testing.md)

## 目次

- [モデルのテスト](#モデルのテスト)
- [リレーションシップのテスト](#リレーションシップのテスト)
- [クエリのテスト](#クエリのテスト)
- [トランザクションのテスト](#トランザクションのテスト)
- [テストデータの管理](#テストデータの管理)
...
class TestUserModel:
    ...
    async def test_read_user(self, db_session):
        ...
        stmt = select(SampleUser).where(SampleUser.id == user.id)
    ...
    async def test_delete_user(self, db_session):
        ...
        # 存在しないことを確認
        from sqlalchemy import select
        stmt = select(SampleUser).where(SampleUser.id == user_id)
        result = await db_session.execute(stmt)
...
class TestUserFileRelationship:
    ...
    async def test_user_files_relationship(self, db_session):
        ...
        stmt = select(SampleUser).where(SampleUser.id == user.id).options(selectinload(User.files))
        result = await db_session.execute(stmt)
    ...
    async def test_cascade_delete(self, db_session):
        ...
        # ファイルも削除されていることを確認
        from sqlalchemy import select
        stmt = select(SampleFile).where(File.id == file_id)
        result = await db_session.execute(stmt)
...
class TestDatabaseQueries:
    ...
    async def test_filter_active_users(self, db_session, sample_users):
        """アクティブユーザーのフィルタリング"""
        stmt = select(SampleUser).where(SampleUser.is_active == True)
        result = await db_session.execute(stmt)
    ...
    async def test_pagination(self, db_session, sample_users):
        """ページネーション"""
        # 最初の2件
        stmt = select(SampleUser).limit(2).offset(0)
        result = await db_session.execute(stmt)
        ...
        # 次の2件
        stmt = select(SampleUser).limit(2).offset(2)
        result = await db_session.execute(stmt)
    ...
    async def test_order_by(self, db_session, sample_users):
        """ソート"""
        # メールアドレスで昇順
        stmt = select(SampleUser).order_by(SampleUser.email.asc())
        result = await db_session.execute(stmt)
        users_asc = result.scalars().all()

        # メールアドレスで降順
        stmt = select(SampleUser).order_by(SampleUser.email.desc())
        result = await db_session.execute(stmt)
...
class TestComplexQueries:
    ...
    async def test_join_query(self, db_session):
        ...
        # JOINでユーザーとファイルを同時取得
        stmt = select(SampleUser).options(joinedload(User.files)).where(SampleUser.id == user.id)
        result = await db_session.execute(stmt)
    ...
    async def test_aggregate_query(self, db_session):
        ...
        from sqlalchemy import select, func
        stmt = (
            select(SampleUser.id, func.count(SampleFile.id).label("file_count"))
            .join(File)
            .group_by(SampleUser.id)
        )
        result = await db_session.execute(stmt)
        counts = result.all()
...
おめでとうございます！データベーステストの実装が完了しました。
