# 機能単位仕様書

本ディレクトリは、CAMPシステムの機能単位での設計仕様書を格納します。

---

## 目次

| No | ディレクトリ | 機能名 | 対象ユースケース | ステータス |
|----|-------------|--------|-----------------|-----------|
| 01 | [01-api-overview/](./01-api-overview/) | API共通仕様 | - | 作成済 |
| 02 | [02-common-ui/](./02-common-ui/) | 共通UI（ヘッダー・サイドバー・検索・通知） | UI-001〜UI-011 | 作成済 |
| 03 | [03-user-management/](./03-user-management/) | ユーザー管理 | U-001〜U-013 | 作成済 |
| 04 | [04-project-management/](./04-project-management/) | プロジェクト管理 | P-001〜PF-006 | 作成済 |
| 05 | [05-analysis/](./05-analysis/) | 個別施策分析 | AVM-001〜AST-006 | 作成済 |
| 06 | [06-driver-tree/](./06-driver-tree/) | ドライバーツリー | DTC-001〜DTP-006 | 作成済 |
| 07 | [07-dashboard/](./07-dashboard/) | ダッシュボード | D-001〜D-006 | 作成済 |
| 08 | [08-template/](./08-template/) | テンプレート | TM-001〜TM-005 | 作成済 |
| 09 | [09-copy-export/](./09-copy-export/) | 複製・エクスポート | CP-001〜EX-003 | 作成済 |
| 10 | [10-file-version/](./10-file-version/) | ファイルバージョン | FV-001〜FV-004 | 作成済 |
| 11 | [11-system-admin/](./11-system-admin/) | システム管理 | SA-001〜SA-043 | 作成済 |

---

## ユースケースカバレッジマトリクス

| カテゴリ | UC数 | カバー済 | カバー率 | 仕様書 |
|---------|------|---------|---------|--------|
| 共通UI（サイドバー・検索・通知） | 11 | 11 | 100% | 02-common-ui |
| ユーザー管理 | 13 | 13 | 100% | 03-user-management |
| プロジェクト管理 | 20 | 20 | 100% | 04-project-management |
| 個別施策分析（マスタ） | 25 | 25 | 100% | 05-analysis |
| 個別施策分析（セッション） | 24 | 24 | 100% | 05-analysis |
| ドライバーツリー | 41 | 41 | 100% | 06-driver-tree |
| ダッシュボード・統計 | 6 | 6 | 100% | 07-dashboard |
| テンプレート機能 | 5 | 5 | 100% | 08-template |
| 複製・エクスポート機能 | 5 | 5 | 100% | 09-copy-export |
| ファイルバージョン管理 | 4 | 4 | 100% | 10-file-version |
| システム管理（管理者専用） | 43 | 43 | 100% | 11-system-admin |
| 横断的ユースケース | 18 | 18 | 100% | （各機能に分散） |
| **合計** | **215** | **215** | **100%** | - |

---

## 仕様書の標準構成

各機能仕様書は以下の構成に従って作成します。

```markdown
# [機能名] 統合設計書（[ユースケースID範囲]）

## 1. 概要
### 1.1 目的
### 1.2 対象ユースケース（表形式）
### 1.3 追加コンポーネント数

## 2. データベース設計
### テーブル定義（カラム、インデックス）

## 3. APIエンドポイント設計
### エンドポイント一覧
### リクエスト/レスポンス定義

## 4. Pydanticスキーマ設計

## 5. サービス層設計

## 6. フロントエンド設計
### 6.1 画面一覧
### 6.2 コンポーネント構成

## 7. 画面項目・APIマッピング
### 7.1 一覧画面
| 画面項目 | 表示形式 | APIエンドポイント | レスポンスフィールド | 変換処理 |

### 7.2 作成/編集画面
| 画面項目 | 入力形式 | 必須 | APIエンドポイント | リクエストフィールド | バリデーション |

## 8. ユースケースカバレッジ表
| UC ID | 機能名 | API | 画面 | ステータス |
```

---

## 関連ドキュメント

- **ユースケース一覧**: [../01-usercases/01-usecases.md](../01-usercases/01-usecases.md)
- **モックアップ**: [../03-mockup/](../03-mockup/)
- **データベース設計**: [../06-database/](../06-database/)

---

### ドキュメント管理情報

- **作成日**: 2026年1月1日
- **更新日**: 2026年1月1日
