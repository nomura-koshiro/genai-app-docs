...
このドキュメントでは、AI Agent Appのデータベース設計について詳しく説明します。
...
async def get_active_users(db: AsyncSession) -> list[User]:
    """アクティブなユーザーの一覧を取得。"""
    query = select(User).where(User.is_active == True).order_by(User.created_at)
    result = await db.execute(query)
...
async def get_user_with_sessions(db: AsyncSession, user_id: int) -> User | None:
    """ユーザーとセッションを一緒に取得。"""
    query = (
        select(User)
        .options(selectinload(User.sessions))  # セッションを一緒に読み込み
...
async def get_sessions_with_messages(
    ...
    query = (
        select(Session)
        .options(
            selectinload(Session.messages),  # メッセージを読み込み
            selectinload(Session.user)       # ユーザーも読み込み
        )
...
async def get_total_storage_size(db: AsyncSession, user_id: int) -> int:
    """ユーザーの総ストレージサイズを取得。"""
    query = select(func.sum(File.size)).where(File.user_id == user_id)
    result = await db.execute(query)
