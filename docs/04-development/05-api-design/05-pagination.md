...
async def list_users(
    ...
    return [SampleUserResponse.model_validate(u) for u in users]
...
async def list_users(
    ...
    大量データに適している。
    """
    query = select(SampleUser).order_by(SampleUser.created_at.desc()).limit(limit)

    if cursor:
        query = query.where(SampleUser.created_at < cursor)
