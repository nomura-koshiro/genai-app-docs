...
async def test_update_last_owner_role(db_session, test_project_with_owner, test_users):
    ...
    manager_member = await service.repository.get_by_project_and_user(project.id, test_users[0].id)
...
async def test_remove_self(db_session, test_project_with_owner, test_users):
    ...
    owner_member = await service.repository.get_by_project_and_user(project.id, test_users[0].id)
...
async def test_remove_last_owner(db_session, test_project_with_owner, test_users):
    ...
    first_owner = await service.repository.get_by_project_and_user(project.id, test_users[0].id)
...
async def test_leave_project_success(db_session, test_project_with_owner, test_users):
    ...
    # Assert
    left_member = await service.repository.get_by_project_and_user(project.id, test_users[1].id)
