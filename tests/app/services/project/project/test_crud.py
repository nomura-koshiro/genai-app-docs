...
async def test_list_user_projects(db_session: AsyncSession):
    """[test_project-007] ユーザーのプロジェクト一覧取得。"""
    # Arrange
    service = ProjectService(db_session)
    user_id = uuid.uuid4()
...
async def test_update_project_success(db_session: AsyncSession):
    """[test_project-008] プロジェクト更新の成功ケース（OWNERロール）。"""
    # Arrange
    service = ProjectService(db_session)
    owner_id = uuid.uuid4()
...
async def test_update_project_permission_denied(db_session: AsyncSession):
    """[test_project-009] 権限のないユーザーによる更新エラー。"""
    # Arrange
    service = ProjectService(db_session)
    owner_id = uuid.uuid4()
...
async def test_delete_project_success(db_session: AsyncSession):
    """[test_project-010] プロジェクト削除の成功ケース（OWNERロール）。"""
    # Arrange
    service = ProjectService(db_session)
    owner_id = uuid.uuid4()
...
async def test_delete_project_permission_denied(db_session: AsyncSession):
    """[test_project-011] 権限のないユーザーによる削除エラー。"""
    # Arrange
    service = ProjectService(db_session)
    owner_id = uuid.uuid4()
    admin_id = uuid.uuid4()
...
async def test_delete_project_without_files(db_session: AsyncSession):
    """[test_project-013] ファイルのないプロジェクトの削除が成功すること。"""
    # Arrange
    service = ProjectService(db_session)
    owner_id = uuid.uuid4()
